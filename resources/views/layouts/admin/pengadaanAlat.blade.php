<x-app-layout>
    ...
    <x-table :columns="$columns" :rows="$rows" rowActions="table-action-buttons" :rowActionsParams="['allowedActions' => ['delete']]">
        ...
        <x-slot name="actions">
            <x-action-button label="AjukanPengadaan" icon="fa-plus" href="{{ route('pengajuanAlat') }}" />
        </x-slot>
    </x-table>
    @else
    <x-form-container action="#">
        ...
        <x-input-field label="Harga" name="harga" type="number" />
        <x-input-field label="Nama Vendor" name="vendor" />
        <x-input-field label="Tujuan Pengadaan" name="tujuan" />
        <x-input-field label="Tanggal Pembelian" name="nama" type="date" />
    </x-form-container>
    @endif
</x-app-layout>